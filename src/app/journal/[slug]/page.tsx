...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const { data: post } = supabase
    ? await supabase.from("blog_posts").select("title, excerpt").eq("slug", (await params).slug).eq("status", "published").maybeSingle()
    : { data: null };
...
export default async function JournalPostPage({ params }: Props) {
  const supabase = createSupabasePublicClient();
  const { data: post } = supabase
    ? await supabase.from("blog_posts").select("title, excerpt, body_markdown, tags, published_at").eq("slug", (await params).slug).eq("status", "published").maybeSingle()
    : { data: null };
  if (!post) notFound();
