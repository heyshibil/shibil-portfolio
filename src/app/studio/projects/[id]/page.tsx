...
export default async function EditProjectPage({
  ...
  searchParams: Promise<{ "cover-removed"?: string; "gallery-removed"?: string; error?: string }>;
}) {
  ...
  if (!admin) notFound();
  const projectQuery = admin.from("projects").select("id, name, slug, project_type, year, summary, scope, stack, challenge, lessons, live_url, github_url, is_published, sort_order, cover_image_path");
  const { data: project } = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(id)
