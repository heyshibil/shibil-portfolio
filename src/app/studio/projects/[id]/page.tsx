...
export default async function EditProjectPage({
  ...
  if (!admin) notFound();
  const { data: project } = await admin.from("projects").select("id, name, slug, project_type, year, summary, scope, stack, challenge, lessons, live_url, github_url, is_published, sort_order, cover_image_path").eq("id", id).maybeSingle();
  if (!project) notFound();
